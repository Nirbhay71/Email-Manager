...
def _ensure_generated():
    """
    Check that gRPC Python stubs exist; generate them if missing, mirroring
    search_feature_demo/grpc_app/server.py's convention so a fresh checkout
    ...
    logger.info("Generating gRPC stubs from classifier.proto ...")
    from grpc_tools import protoc

    proto_dir = os.path.join(_SERVICE_DIR, "protos")
    proto_path = os.path.join(proto_dir, "classifier.proto")
    result = protoc.main(
        [
            "grpc_tools.protoc",
            f"-I{proto_dir}",
...
# Same shared-secret gRPC interceptor pattern as search_feature_demo/grpc_app/server.py.
...
class _ServiceTokenInterceptor(grpc.ServerInterceptor):
    ...
    All 4 RPCs on this service are unary_unary, so only that handler shape
    needs a deny path (unlike search_feature_demo's interceptor, which also
    has to cover AskQuestion's server-streaming RPC).
    """
...
def serve():
    ...
    # Bind to localhost only — reached exclusively by the Node backend running
    # on the same host/private network, same convention as search_feature_demo.
